...
# Loading Images
cue_ball_image = pygame.Surface((ball_diameter, ball_diameter), pygame.SRCALPHA)
pygame.draw.circle(cue_ball_image, (255, 255, 255), (ball_diameter // 2, ball_diameter // 2), ball_diameter // 2)
cue_stick_image = pygame.image.load("images\cue_stick.png").convert_alpha()
pool_balls_image = pygame.image.load("images\pool_balls.png").convert_alpha()
...
cue_ball = create_ball(ball_diameter / 2, pos, (255, 255, 255))
...
while gameRun:
    ...
    for ball in balls:
        ...
        for pos in hole_positions:
            if math.sqrt((ballPos[0] - pos[0]) ** 2 + (ballPos[1] - pos[1]) ** 2) <= ball_diameter:
